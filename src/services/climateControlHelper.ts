...
export class ClimateControlHelper {
    ...
    getSetpoint(operationMode: DaikinOperationModes): DaikinTemperatureControlSetpoints {
        ...
        if (setpointMode) {
            ...
            throw new Error(
                `Could not determine the TemperatureControlSetpoint for operationMode: ${operationMode}, setpointMode: ${setpointMode}, controlMode: ${controlMode}, for device: ${JSON.stringify(DaikinCloudRepo.maskSensitiveCloudDeviceData(this.device.desc), null, 4)}`,
            );
        }
    ...
    hasSwingModeVerticalFeature() {
        const verticalSwing = this.getData(
            'fanControl',
            `/operationModes/${this.getCurrentOperationMode()}/fanDirection/vertical/currentMode`,
        );
        this.platform.daikinLogger.debug(`[${this.name}] hasSwingModeFeature, verticalSwing: ${Boolean(verticalSwing)}`);
        return Boolean(verticalSwing);
    }

    hasSwingModeHorizontalFeature() {
        const horizontalSwing = this.getData(
            'fanControl',
            `/operationModes/${this.getCurrentOperationMode()}/fanDirection/horizontal/currentMode`,
        );
        this.platform.daikinLogger.debug(`[${this.name}] hasSwingModeFeature, horizontalSwing: ${Boolean(horizontalSwing)}`);
    ...
    hasPowerfulModeFeature() {
        ...
        this.platform.daikinLogger.debug(`[${this.name}] hasPowerfulModeFeature, powerfulMode: ${Boolean(powerfulMode)}`);
        return Boolean(powerfulMode);
    }

    hasEconoModeFeature() {
        const econoMode = this.getData('econoMode', undefined);
        this.platform.daikinLogger.debug(`[${this.name}] hasEconoModeFeature, econoMode: ${Boolean(econoMode)}`);
        return Boolean(econoMode);
    }

    hasStreamerModeFeature() {
        const streamerMode = this.getData('streamerMode', undefined);
        this.platform.daikinLogger.debug(`[${this.name}] hasStreamerModeFeature, streamerMode: ${Boolean(streamerMode)}`);
    ...
    hasOutdoorSilentModeFeature() {
        ...
        this.platform.daikinLogger.debug(
            `[${this.name}] hasOutdoorSilentModeFeature, outdoorSilentMode: ${Boolean(OutdoorSilentMode)}`,
        );
    ...
    hasIndoorSilentModeFeature() {
        ...
        this.platform.daikinLogger.debug(
            `[${this.name}] hasIndoorSilentModeFeature, indoorSilentMode: ${fanSpeedValues.includes(DaikinFanSpeedModes.QUIET)}`,
        );
    ...
    hasOperationMode(operationMode: DaikinOperationModes) {
        ...
        const operationModeValues: Array<string> =
            operationModeData && operationModeData.values ? operationModeData.values : [];
        this.platform.daikinLogger.debug(`[${this.name}] has ${operationMode}: ${operationModeValues.includes(operationMode)}`);
