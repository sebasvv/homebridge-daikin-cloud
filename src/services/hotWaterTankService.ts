...
export class HotWaterTankService extends BaseService {
    ...
    constructor(
        ...
        this.hotWaterTankService =
            this.accessory.getService('Hot water tank') ||
            accessory.addService(this.platform.Service.Thermostat, 'Hot water tank', 'hot_water_tank');
        this.hotWaterTankService.setCharacteristic(this.platform.Characteristic.Name, 'Hot water tank');

        this.hotWaterTankService.addOptionalCharacteristic(this.platform.Characteristic.ConfiguredName);
        this.hotWaterTankService.setCharacteristic(this.platform.Characteristic.ConfiguredName, 'Hot water tank');
        ...
        const temperatureControl = this.getData(
            'temperatureControl',
            '/operationModes/heating/setpoints/domesticHotWaterTemperature',
        );
        const targetTemperature = this.hotWaterTankService.getCharacteristic(
    ...
    async handleHotWaterTankCurrentHeatingCoolingStateGet(): Promise<CharacteristicValue> {
        ...
        this.platform.daikinLogger.debug(
            `[${this.name}] GET ActiveState, state: ${state}, last update: ${this.accessory.context.device.getLastUpdated()}`,
        );
        const val =
            state === DaikinOnOffModes.ON
    ...
    async handleHotWaterTankCurrentTemperatureGet(): Promise<CharacteristicValue> {
        ...
        this.platform.daikinLogger.debug(
            `[${this.name}] GET CurrentTemperature for hot water tank, temperature: ${temperature}`,
        );
        return temperature;
    }
    ...
    async handleHotWaterTankHeatingTargetTemperatureGet(): Promise<CharacteristicValue> {
        ...
        this.platform.daikinLogger.debug(
            `[${this.name}] GET HeatingThresholdTemperature domesticHotWaterTank, temperature: ${temperature}`,
        );
        return temperature;
    }
    ...
    async handleHotWaterTankHeatingTargetTemperatureSet(value: CharacteristicValue) {
        ...
        this.platform.daikinLogger.debug(
            `[${this.name}] SET HeatingTargetTemperature domesticHotWaterTank, temperature to: ${temperature}`,
        );
    ...
    async handleHotWaterTankTargetHeatingCoolingStateGet(): Promise<CharacteristicValue> {
        const operationMode: DaikinOperationModes = this.safeGetValue<DaikinOperationModes>(
            'operationMode',
            undefined,
            DaikinOperationModes.AUTO,
        );
        const state = this.safeGetValue('onOffMode', undefined);
        this.platform.daikinLogger.debug(
            `[${this.name}] GET TankTargetHeatingCoolingState, operationMode: ${operationMode}, state: ${state}`,
        );
    ...
    async handleHotWaterTankTargetHeatingCoolingStateSet(value: CharacteristicValue) {
        ...
        this.platform.daikinLogger.debug(
            `[${this.name}] SET TargetHeatingCoolingState, daikinOperationMode to: ${daikinOperationMode}`,
        );
    ...
    async handlePowerfulModeGet(): Promise<boolean> {
        ...
        this.platform.daikinLogger.debug(
            `[${this.name}] GET PowerfulMode, powerfulModeOn: ${powerfulModeOn}, last update: ${this.accessory.context.device.getLastUpdated()}`,
        );
