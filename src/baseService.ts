...
import { DaikinCloudRepo, DaikinDataPoint } from './repository/daikinCloudRepo';
...
export abstract class BaseService {
    ...
    protected safeGetValue<T = unknown>(key: string, path: string | undefined, defaultValue: T = 0 as unknown as T): T {
        return DaikinCloudRepo.safeGetValue<T>(this.accessory.context.device, this.managementPointId, key, path, defaultValue);
    }

    protected async safeSetData(key: string, path: string | undefined, value: unknown) {
        try {
            await this.accessory.context.device.setData(this.managementPointId, key, path === undefined ? null : path, value);
        } catch (e) {
            this.platform.log.error(`[${this.name}] Failed to set ${key}${path ? ' ' + path : ''}`, e, JSON.stringify(DaikinCloudRepo.maskSensitiveCloudDeviceData(this.accessory.context.device.desc), null, 4));
        }
    }
}
