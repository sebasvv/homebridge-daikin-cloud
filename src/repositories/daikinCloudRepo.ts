...
export class DaikinCloudRepo {
    static getData(
        ...
        managementPointId: string | null,
        key: string,
        path?: string,
    ): DaikinDataPoint | undefined {
        return device.getData(managementPointId, key, path);
    }

    static safeGetValue<T = unknown>(
        device: DaikinCloudDevice,
        managementPointId: string | null,
        key: string,
        path?: string,
    ...
    static maskSensitiveCloudDeviceData(cloudDeviceDetails: DaikinCloudDeviceDetails) {
        return {
            ...
            managementPoints: cloudDeviceDetails.managementPoints.map((managementPoint) => {
                ...
                if (masked.ipAddress) {
                    masked.ipAddress.value = 'REDACTED';
                }
                if (masked.macAddress) {
                    masked.macAddress.value = 'REDACTED';
                }
                if (masked.ssid) {
                    masked.ssid.value = 'REDACTED';
                }
                if (masked.serialNumber) {
                    masked.serialNumber.value = 'REDACTED';
                }
                if (masked.wifiConnectionSSID) {
                    masked.wifiConnectionSSID.value = 'REDACTED';
                }
                if (masked.consumptionData) {
                    masked.consumptionData = 'REDACTED';
                }
                if (masked.schedule) {
                    masked.schedule = 'REDACTED';
                }
