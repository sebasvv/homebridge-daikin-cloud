...
import {
    DaikinEconoModes, DaikinFanDirectionHorizontalModes,
    DaikinFanDirectionVerticalModes, DaikinFanSpeedModes,
    DaikinOnOffModes,
    DaikinOperationModes, DaikinOutdoorSilentModes, DaikinPowerfulModes, DaikinStreamerModes,
} from './baseService';
...
export class ClimateControlHandlers {
    ...
    private async safeSetData(key: string, path: string | undefined, value: unknown) {
        try {
            ...
            this.platform.log.error(`[${this.name}] Error setting data for ${key}: ${e}`);
        }
    }
    ...
    async handleCoolingThresholdTemperatureGet(): Promise<CharacteristicValue> {
        const temperature = this.safeGetValue<number>('temperatureControl', `/operationModes/${DaikinOperationModes.COOLING}/setpoints/${this.helper.getSetpoint(DaikinOperationModes.COOLING)}`);
        this.platform.log.debug(`[${this.name}] GET CoolingThresholdTemperature, temperature: ${temperature}, last update: ${this.device.getLastUpdated()}`);
        return temperature;
    }
    ...
    async handleCoolingThresholdTemperatureSet(value: CharacteristicValue) {
        ...
        await this.safeSetData('temperatureControl', `/operationModes/${DaikinOperationModes.COOLING}/setpoints/${this.helper.getSetpoint(DaikinOperationModes.COOLING)}`, temperature);
        this.forceUpdateDevices();
    }

    async handleRotationSpeedGet(): Promise<CharacteristicValue> {
        const speed = this.safeGetValue<number>('fanControl', `/operationModes/${this.helper.getCurrentOperationMode()}/fanSpeed/modes/fixed`);
        this.platform.log.debug(`[${this.name}] GET RotationSpeed, speed: ${speed}, last update: ${this.device.getLastUpdated()}`);
        return speed;
    }
    ...
    async handleRotationSpeedSet(value: CharacteristicValue) {
        ...
        await this.safeSetData('fanControl', `/operationModes/${this.helper.getCurrentOperationMode()}/fanSpeed/currentMode`, 'fixed');
        await this.safeSetData('fanControl', `/operationModes/${this.helper.getCurrentOperationMode()}/fanSpeed/modes/fixed`, speed);
        this.forceUpdateDevices();
    }

    async handleHeatingThresholdTemperatureGet(): Promise<CharacteristicValue> {
        const temperature = this.safeGetValue<number>('temperatureControl', `/operationModes/${DaikinOperationModes.HEATING}/setpoints/${this.helper.getSetpoint(DaikinOperationModes.HEATING)}`);
        this.platform.log.debug(`[${this.name}] GET HeatingThresholdTemperature, temperature: ${temperature}, last update: ${this.device.getLastUpdated()}`);
        return temperature;
    }
    ...
    async handleHeatingThresholdTemperatureSet(value: CharacteristicValue) {
        ...
        await this.safeSetData('temperatureControl', `/operationModes/${DaikinOperationModes.HEATING}/setpoints/${this.helper.getSetpoint(DaikinOperationModes.HEATING)}`, temperature);
        this.forceUpdateDevices();
    }
    ...
    async handleTargetHeaterCoolerStateGet(): Promise<CharacteristicValue> {
        ...
        this.platform.log.debug(`[${this.name}] GET TargetHeaterCoolerState, operationMode: ${operationMode}, last update: ${this.device.getLastUpdated()}`);
    ...
    async handleSwingModeSet(value: CharacteristicValue) {
        ...
        if (this.helper.hasSwingModeHorizontalFeature()) {
            await this.safeSetData('fanControl', `/operationModes/${this.helper.getCurrentOperationMode()}/fanDirection/horizontal/currentMode`, daikinSwingMode);
        }

        if (this.helper.hasSwingModeVerticalFeature()) {
            await this.safeSetData('fanControl', `/operationModes/${this.helper.getCurrentOperationMode()}/fanDirection/vertical/currentMode`, daikinSwingMode);
        }
    ...
    async handleSwingModeGet(): Promise<CharacteristicValue> {
        const verticalSwingMode = this.helper.hasSwingModeVerticalFeature() ? this.safeGetValue<string | null>('fanControl', `/operationModes/${this.helper.getCurrentOperationMode()}/fanDirection/vertical/currentMode`, null) : null;
        const horizontalSwingMode = this.helper.hasSwingModeHorizontalFeature() ? this.safeGetValue<string | null>('fanControl', `/operationModes/${this.helper.getCurrentOperationMode()}/fanDirection/horizontal/currentMode`, null) : null;
        this.platform.log.debug(`[${this.name}] GET SwingMode, verticalSwingMode: ${verticalSwingMode}, last update: ${this.device.getLastUpdated()}`);
        this.platform.log.debug(`[${this.name}] GET SwingMode, horizontalSwingMode: ${horizontalSwingMode}, last update: ${this.device.getLastUpdated()}`);
    ...
    async handlePowerfulModeGet() {
        const powerfulModeOn = this.safeGetValue<DaikinPowerfulModes>('powerfulMode', undefined) === DaikinPowerfulModes.ON;
        this.platform.log.debug(`[${this.name}] GET PowerfulMode, powerfulModeOn: ${powerfulModeOn}, last update: ${this.device.getLastUpdated()}`);
    ...
    async handleEconoModeGet() {
        ...
        this.platform.log.debug(`[${this.name}] GET EconoMode, econoModeOn: ${econoModeOn}, last update: ${this.device.getLastUpdated()}`);
        return econoModeOn;
    }
    ...
    async handleStreamerModeGet() {
        const streamerModeOn = this.safeGetValue<DaikinStreamerModes>('streamerMode', undefined) === DaikinStreamerModes.ON;
        this.platform.log.debug(`[${this.name}] GET StreamerMode, streamerModeOn: ${streamerModeOn}, last update: ${this.device.getLastUpdated()}`);
    ...
    async handleOutdoorSilentModeGet() {
        const outdoorSilentModeOn = this.safeGetValue<DaikinOutdoorSilentModes>('outdoorSilentMode', undefined) === DaikinOutdoorSilentModes.ON;
        this.platform.log.debug(`[${this.name}] GET OutdoorSilentMode, outdoorSilentModeOn: ${outdoorSilentModeOn}, last update: ${this.device.getLastUpdated()}`);
    ...
    async handleIndoorSilentModeGet() {
        const indoorSilentModeOn = this.safeGetValue<DaikinFanSpeedModes>('fanControl', `/operationModes/${this.helper.getCurrentOperationMode()}/fanSpeed/currentMode`) === DaikinFanSpeedModes.QUIET;
        this.platform.log.debug(`[${this.name}] GET IndoorSilentMode, indoorSilentModeOn: ${indoorSilentModeOn}, last update: ${this.device.getLastUpdated()}`);
    ...
    async handleIndoorSilentModeSet(value: CharacteristicValue) {
        ...
        await this.safeSetData('fanControl', `/operationModes/${this.helper.getCurrentOperationMode()}/fanSpeed/currentMode`, daikinFanSpeedMode);
        this.forceUpdateDevices();
    }

    async handleDryOperationModeGet() {
        const dryOperationModeOn = this.safeGetValue<DaikinOperationModes>('operationMode', undefined) === DaikinOperationModes.DRY;
        this.platform.log.debug(`[${this.name}] GET DryOperationMode, dryOperationModeOn: ${dryOperationModeOn}, last update: ${this.device.getLastUpdated()}`);
    ...
    async handleFanOnlyOperationModeGet() {
        const fanOnlyOperationModeOn = this.safeGetValue<DaikinOperationModes>('operationMode', undefined) === DaikinOperationModes.FAN_ONLY;
        this.platform.log.debug(`[${this.name}] GET FanOnlyOperationMode, fanOnlyOperationModeOn: ${fanOnlyOperationModeOn}, last update: ${this.device.getLastUpdated()}`);
